...
describe("Teste de API Produtos", function () {
  test("POST / deve retornar 201 um objeto JSON", async function () {
    ...
    expect(response.type).toBe("application/json") //nome do cabeçalho
  ...
  test("GET / deve retornar 200 um array JSON", async function () {
    ...
    expect(response.type).toBe("application/json") //nome do cabeçalho
    expect(Array.isArray(response.body)).toBe(true); //objeto array se é array mesmo e devolve o retorno do array do controller_produtos.js
